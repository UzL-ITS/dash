#ifndef CUDA_AES_ENGINE_H
#define CUDA_AES_ENGINE_H
...
__constant__ u32 Te0_device[256];
__constant__ u32 Te1_device[256];
__constant__ u32 Te2_device[256];
__constant__ u32 Te3_device[256];

__constant__ unsigned int rd_enc_key_device[4 * (ROUNDS + 1)];
 ...
 */
__device__ void cuda_cipher(__uint128_t *ciphertext, __uint128_t *plaintext) {
    unsigned char *in = (unsigned char *)plaintext;
    unsigned char *out = (unsigned char *)ciphertext;
...
class CUDAAESEngine {
   ...
   public:
    CUDAAESEngine() {
        ...
        AES_set_encrypt_key(m_fixed_key, 128, &m_key_schedule);
        // Copy t-tables to device
        cudaCheckError(cudaMemcpyToSymbol(Te0_device, Te0, 256 * sizeof(u32)));
        cudaCheckError(cudaMemcpyToSymbol(Te1_device, Te1, 256 * sizeof(u32)));
        cudaCheckError(cudaMemcpyToSymbol(Te2_device, Te2, 256 * sizeof(u32)));
        cudaCheckError(cudaMemcpyToSymbol(Te3_device, Te3, 256 * sizeof(u32)));
        // Copy round-keys for the encryption to device
        cudaCheckError(
            cudaMemcpyToSymbol(rd_enc_key_device, m_key_schedule.rk,
                               4 * (ROUNDS + 1) * sizeof(unsigned int)));
    }
};
